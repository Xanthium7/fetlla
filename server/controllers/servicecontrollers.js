...
export const getallservices = async (req, res) => {
  try {
    const allservices = await servicemodel.find().sort({ updatedAt: -1 });
    res.status(200).json({ status: true, allservices });
  } catch (er) {
    res.status(500).json({ status: false, msg: "failed" });
  }
};
export const delserviceId = async (req,res)=>{
    try {
      const allservice = await servicemodel.deleteOne({_id:req.query.id})
      const remainingservice = await servicemodel.find().sort({updatedAt:-1})
      res.status(200).json({status:true,allservices:remainingservice})
